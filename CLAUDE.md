...
- **Party action rate limiting**: `createParty`, `joinParty`, and `resumeParty` are rate-limited per socket id (5 actions / 10 s); `joinParty` and `resumeParty`, which accept an invite code, are additionally limited per client IP (60 / 60 s, disabled when `E2E_TESTS=1`) so opening new sockets can't bypass the limit. Invite codes come from `crypto.randomInt`. Uses `checkFixedWindowRateLimit` from `apps/platform/server/observability/rateLimit.ts`. Reset in tests via `resetPartyActionRateLimit()`.
- **In-match gameplay rate limiting**: high-frequency or otherwise attacker-shaped gameplay events are rate-limited per socket via `createSocketRateLimiter` (also from `observability/rateLimit.ts`) — e.g. Kritzelagent's `submitStroke` (10/s), and the shared gameplay limiter used by Flip 7 (`hit`/`stay`/`chooseActionTarget`), Secret Signals (`focusCard`/`giveSignal`/`revealCard`), and Imposter (`submitDescription`/`submitVote`) at 20/s; Imposter's `submitWord` has its own 5 / 10 s limit. See `docs/adding-a-new-game.md`'s "Rate Limiting" section for the pattern and when a new event needs one.
...
- **Word persistence** (Imposter): `IMPOSTER_PERSIST_WORDS` env flag (default `true`) controls whether submitted words are written to `words.<locale>.txt` (per match language). Set to `false` to keep submitted words in memory only (e.g. to stop players from adding words to the shared library). `IMPOSTER_WORDS_DIR` moves the files to a mounted volume (`docker-compose.yml` mounts `imposter-words`) so words survive container replacement. At most `WORD_LIBRARY_MAX_SIZE` (2000) player-submitted words are kept (bundled words don't count); further submissions are dropped. `submitWord` is rate-limited (5 / 10 s per socket) and rejects control characters, because the library is shared by every future room.
